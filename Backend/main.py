...
def serve_frontend(path):
    ...
    if path.endswith('/'):
        ...
        if os.path.isfile(dir_index):
            print(f"Servendo index.html dopo aver rimosso lo slash finale: {dir_index}")  # Log per debug
            return send_from_directory(os.path.join(app.static_folder, adjusted_path), 'index.html')
    
    # Per SPA, servi index.html dalla radice per tutte le altre richieste
    # Next.js gestirà il routing lato client
    print(f"Fallback: servendo index.html dalla root per il percorso: {path}")  # Log per debug
...
def get_user_cvs(email):
    ...
    try:
        ...
        print(f"Errore durante il recupero dei CV per l'utente {email}: {str(e)}")  # Log per debug
        return jsonify({
            "error": str(e),
